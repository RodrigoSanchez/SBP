 ...
 */(function(window, undefined) {
  function createFlags(flags) {
    ...
    flags = flags.split(/\s+/);
    for (i = 0, length = flags.length; i < length; i++) object[flags[i]] = !0;
    return object;
  }
  function dataAttr(elem, key, data) {
    if (data === undefined && elem.nodeType === 1) {
      ...
      if (typeof data == "string") {
        try {
          data = data === "true" ? !0 : data === "false" ? !1 : data === "null" ? null : jQuery.isNumeric(data) ? +data : rbrace.test(data) ? jQuery.parseJSON(data) : data;
        } catch (e) {}
  ...
  function handleQueueMarkDefer(elem, type, src) {
    ...
    defer && (src === "queue" || !jQuery._data(elem, queueDataKey)) && (src === "mark" || !jQuery._data(elem, markDataKey)) && setTimeout(function() {
      if (!jQuery._data(elem, queueDataKey) && !jQuery._data(elem, markDataKey)) {
        jQuery.removeData(elem, deferDataKey, !0);
        defer.fire();
      }
    }, 0);
  }
  ...
  function cloneFixAttributes(src, dest) {
    ...
    if (nodeName === "object") dest.outerHTML = src.outerHTML; else if (nodeName !== "input" || src.type !== "checkbox" && src.type !== "radio") nodeName === "option" ? dest.selected = src.defaultSelected : nodeName === "input" || nodeName === "textarea" ? dest.defaultValue = src.defaultValue : nodeName === "script" && dest.text !== src.text && (dest.text = src.text); else {
  ...
  function getWidthOrHeight(elem, name, extra) {
    ...
    if (val > 0) {
      if (extra !== "border") for (; i < len; i += 2) {
        extra || (val -= parseFloat(jQuery.css(elem, "padding" + cssExpand[i])) || 0);
        extra === "margin" ? val += parseFloat(jQuery.css(elem, extra + cssExpand[i])) || 0 : val -= parseFloat(jQuery.css(elem, "border" + cssExpand[i] + "Width")) || 0;
      }
      return val + "px";
    }
    ...
    if (extra) for (; i < len; i += 2) {
      ...
      extra !== "padding" && (val += parseFloat(jQuery.css(elem, "border" + cssExpand[i] + "Width")) || 0);
      extra === "margin" && (val += parseFloat(jQuery.css(elem, extra + cssExpand[i])) || 0);
    }
    return val + "px";
  }
  ...
  function buildParams(prefix, obj, traditional, add) {
    ...
    }); else if (!traditional && jQuery.type(obj) === "object") for (var name in obj) buildParams(prefix + "[" + name + "]", obj[name], traditional, add); else add(prefix, obj);
  }
  ...
  function ajaxConvert(s, response) {
    ...
    for (i = 1; i < length; i++) {
      ...
      if (current === "*") current = prev; else if (prev !== "*" && prev !== current) {
          ...
          for (conv1 in converters) {
            ...
            if (tmp[0] === prev || tmp[0] === "*") {
              ...
              if (conv2) {
                ...
                conv1 === !0 ? conv = conv2 : conv2 === !0 && (conv = conv1);
                break;
              }
            }
          }
        ...
        conv !== !0 && (response = conv ? conv(response) : conv2(conv1(response)));
      }
    }
    return response;
  }
    ...
    jQuery.extend({
      ...
      each: function(object, callback, args) {
        ...
        if (args) {
          ...
          } else for (; i < length; ) if (callback.apply(object[i++], args) === !1) break;
        } else if (isObj) {
          for (name in object) if (callback.call(object[name], name, object[name]) === !1) break;
        } else for (; i < length; ) if (callback.call(object[i], i, object[i++]) === !1) break;
        return object;
      },
      ...
      merge: function(first, second) {
        ...
        if (typeof second.length == "number") for (var l = second.length; j < l; j++) first[i++] = second[j]; else while (second[j] !== undefined) first[i++] = second[j++];
        first.length = i;
        return first;
      },
  ...
  jQuery.Callbacks = function(flags) {
    ...
    var list = [], stack = [], memory, fired, firing, firingStart, firingLength, firingIndex, add = function(args) {
      ...
      for (; list && firingIndex < firingLength; firingIndex++) if (list[firingIndex].apply(context, args) === !1 && flags.stopOnFalse) {
        memory = !0;
        break;
      }
      firing = !1;
      ...
      } else memory === !0 ? self.disable() : list = [];
    }, self = {
      add: function() {
        if (list) {
          var length = list.length;
          add(arguments);
          if (firing) firingLength = list.length; else if (memory && memory !== !0) {
            firingStart = length;
      ...
      has: function(fn) {
        if (list) {
          ...
          for (; i < length; i++) if (fn === list[i]) return !0;
        }
        return !1;
      },
      empty: function() {
  ...
  jQuery.extend({
    ...
    when: function(firstParam) {
      ...
      if (length > 1) {
        for (; i < length; i++) args[i] && args[i].promise && jQuery.isFunction(args[i].promise) ? args[i].promise().then(resolveFunc(i), deferred.reject, progressFunc(i)) : --count;
        count || deferred.resolveWith(deferred, args);
  ...
  jQuery.support = function() {
    ...
    jQuery(function() {
      ...
      typeof container.style.zoom != "undefined" && (container.style.zoom = 1);
  ...
  jQuery.extend({
    ...
    acceptData: function(elem) {
      if (elem.nodeName) {
        ...
        if (match) return match !== !0 && elem.getAttribute("classid") === match;
      }
      return !0;
    }
  });
  ...
  jQuery.extend({
    ...
    _unmark: function(force, elem, type) {
      ...
      if (elem) {
        ...
        if (count) jQuery._data(elem, key, count); else {
  ...
  jQuery.fn.extend({
    queue: function(type, data) {
      ...
      return arguments.length < setter ? jQuery.queue(this[0], type) : data === undefined ? this : this.each(function() {
    ...
    promise: function(type, object) {
      ...
      while (i--) if (tmp = jQuery.data(elements[i], deferDataKey, undefined, !0) || (jQuery.data(elements[i], queueDataKey, undefined, !0) || jQuery.data(elements[i], markDataKey, undefined, !0)) && jQuery.data(elements[i], deferDataKey, jQuery.Callbacks("once memory"), !0)) {
        count++;
        tmp.add(resolve);
      }
      resolve();
  ...
  jQuery.extend({
    ...
    attr: function(elem, name, value, pass) {
      ...
      if (hooks && "get" in hooks && notxml && (ret = hooks.get(elem, name)) !== null) return ret;
  ...
  boolHook = {
    ...
    set: function(elem, value, name) {
      var propName;
      if (value === !1) jQuery.removeAttr(elem, name); else {
        propName = jQuery.propFix[name] || name;
        propName in elem && (elem[propName] = !0);
  ...
  jQuery.event = {
    ...
    remove: function(elem, types, handler, selector, mappedTypes) {
      ...
      for (t = 0; t < types.length; t++) {
        ...
        if (eventType.length === 0 && origCount !== eventType.length) {
          (!special.teardown || special.teardown.call(elem, namespaces) === !1) && jQuery.removeEvent(elem, type, elemData.handle);
          delete events[type];
        }
      }
    ...
    trigger: function(event, data, elem, onlyHandlers) {
      if (!elem || elem.nodeType !== 3 && elem.nodeType !== 8) {
        ...
        for (i = 0; i < eventPath.length && !event.isPropagationStopped(); i++) {
          ...
          handle && jQuery.acceptData(cur) && handle.apply(cur, data) === !1 && event.preventDefault();
        }
        event.type = type;
        if (!onlyHandlers && !event.isDefaultPrevented() && (!special._default || special._default.apply(elem.ownerDocument, data) === !1) && (type !== "click" || !jQuery.nodeName(elem, "a")) && jQuery.acceptData(elem) && ontype && elem[type] && (type !== "focus" && type !== "blur" || event.target.offsetWidth !== 0) && !jQuery.isWindow(elem)) {
          old = elem[ontype];
    ...
    dispatch: function(event) {
      ...
      if (delegateCount && (!event.button || event.type !== "click")) {
        ...
        for (cur = event.target; cur != this; cur = cur.parentNode || this) if (cur.disabled !== !0) {
          selMatch = {};
          matches = [];
          jqcur[0] = cur;
  ...
  jQuery.Event = function(src, props) {
    ...
    if (src && src.type) {
      ...
      this.type = src.type;
      this.isDefaultPrevented = src.defaultPrevented || src.returnValue === !1 || src.getPreventDefault && src.getPreventDefault() ? returnTrue : returnFalse;
    } else this.type = src;
  ...
  (function() {
    ...
    var Sizzle = function(selector, context, results, seed) {
      ...
      if (parts.length > 1 && origPOS.exec(selector)) if (parts.length === 2 && Expr.relative[parts[0]]) set = posProcess(parts[0] + parts[1], context, seed); else {
        ...
        if (context) {
          ret = seed ? {
            expr: parts.pop(),
            set: makeArray(seed)
          } : Sizzle.find(parts.pop(), parts.length !== 1 || parts[0] !== "~" && parts[0] !== "+" || !context.parentNode ? context : context.parentNode, contextXML);
      ...
      if (toString.call(checkSet) === "[object Array]") if (!prune) results.push.apply(results, checkSet); else if (context && context.nodeType === 1) for (i = 0; checkSet[i] != null; i++) checkSet[i] && (checkSet[i] === !0 || checkSet[i].nodeType === 1 && Sizzle.contains(context, checkSet[i])) && results.push(set[i]); else for (i = 0; checkSet[i] != null; i++) checkSet[i] && checkSet[i].nodeType === 1 && results.push(set[i]); else makeArray(checkSet, results);
      if (extra) {
    ...
    Sizzle.uniqueSort = function(results) {
      if (sortOrder) {
        ...
        if (hasDuplicate) for (var i = 1; i < results.length; i++) results[i] === results[i - 1] && results.splice(i--, 1);
      }
      return results;
    };
      ...
      while (expr && set.length) {
        for (type in Expr.filter) if ((match = Expr.leftMatch[type].exec(expr)) != null && match[2]) {
          ...
          if (Expr.preFilter[type]) {
            ...
            if (!match) anyFound = found = !0; else if (match === !0) continue;
          }
          if (match) for (i = 0; (item = curLoop[i]) != null; i++) if (item) {
            found = filter(item, match, i, curLoop);
            pass = not ^ found;
            if (inplace && found != null) pass ? anyFound = !0 : curLoop[i] = !1; else if (pass) {
              result.push(item);
              anyFound = !0;
            }
          }
    ...
    var getText = Sizzle.getText = function(elem) {
      ...
      if (nodeType) {
        if (nodeType === 1 || nodeType === 9 || nodeType === 11) {
          ...
          if (typeof elem.innerText == "string") return elem.innerText.replace(rReturn, "");
      ...
      find: {
        ...
        NAME: function(match, context) {
          if (typeof context.getElementsByName != "undefined") {
            ...
            for (var i = 0, l = results.length; i < l; i++) results[i].getAttribute("name") === match[1] && ret.push(results[i]);
      ...
      preFilter: {
        CLASS: function(match, curLoop, inplace, result, not, isXML) {
          ...
          for (var i = 0, elem; (elem = curLoop[i]) != null; i++) elem && (not ^ (elem.className && (" " + elem.className + " ").replace(/[\t\n\r]/g, " ").indexOf(match) >= 0) ? inplace || result.push(elem) : inplace && (curLoop[i] = !1));
          return !1;
        },
        ID: function(match) {
      ...
      filter: {
        PSEUDO: function(elem, match, i, array) {
          ...
          if (name === "contains") return (elem.textContent || elem.innerText || getText([ elem ]) || "").indexOf(match[3]) >= 0;
          if (name === "not") {
            var not = match[3];
            for (var j = 0, l = not.length; j < l; j++) if (not[j] === elem) return !1;
            return !0;
          }
          Sizzle.error(name);
        },
          ...
          switch (type) {
           ...
           case "nth":
            ...
            if (parent && (parent[expando] !== doneName || !elem.nodeIndex)) {
              count = 0;
              for (node = parent.firstChild; node; node = node.nextSibling) node.nodeType === 1 && (node.nodeIndex = ++count);
        ...
        ATTR: function(elem, match) {
          ...
          return result == null ? type === "!=" : !type && Sizzle.attr ? result != null : type === "=" ? value === check : type === "*=" ? value.indexOf(check) >= 0 : type === "~=" ? (" " + value + " ").indexOf(check) >= 0 : check ? type === "!=" ? value !== check : type === "^=" ? value.indexOf(check) === 0 : type === "$=" ? value.substr(value.length - check.length) === check : type === "|=" ? value === check || value.substr(0, check.length + 1) === check + "-" : !1 : value && result !== !1;
        },
  ...
  jQuery.fn.extend({
    ...
    has: function(target) {
      ...
      return this.filter(function() {
        for (var i = 0, l = targets.length; i < l; i++) if (jQuery.contains(this, targets[i])) return !0;
      });
    },
    ...
    closest: function(selectors, context) {
      ...
      if (jQuery.isArray(selectors)) {
        ...
        while (cur && cur.ownerDocument && cur !== context) {
          for (i = 0; i < selectors.length; i++) jQuery(cur).is(selectors[i]) && ret.push({
  ...
  jQuery.fn.extend({
    ...
    before: function() {
      if (this[0] && this[0].parentNode) return this.domManip(arguments, !1, function(elem) {
    ...
    remove: function(selector, keepData) {
      for (var i = 0, elem; (elem = this[i]) != null; i++) if (!selector || jQuery.filter(selector, [ elem ]).length) {
    ...
    domManip: function(args, table, callback) {
      ...
      if (this[0]) {
        ...
        if (first) {
          ...
          for (var i = 0, l = this.length, lastIndex = l - 1; i < l; i++) callback.call(table ? root(this[i], first) : this[i], results.cacheable || l > 1 && i < lastIndex ? jQuery.clone(fragment, !0, !0) : fragment);
        }
        scripts.length && jQuery.each(scripts, function(i, elem) {
  ...
  jQuery.extend({
    clone: function(elem, dataAndEvents, deepDataAndEvents) {
      var srcElements, destElements, i, clone = jQuery.support.html5Clone || jQuery.isXMLDoc(elem) || !rnoshimcache.test("<" + elem.nodeName + ">") ? elem.cloneNode(!0) : shimCloneNode(elem);
    ...
    clean: function(elems, context, fragment, scripts) {
        ...
        if (typeof elem == "string") if (!rhtml.test(elem)) elem = context.createTextNode(elem); else {
          ...
          if (div) {
            ...
            if (safeChildNodes.length > 0) {
              ...
              remove && remove.parentNode && remove.parentNode.removeChild(remove);
            }
          }
        }
        var len;
        if (!jQuery.support.appendChecked) if (elem[0] && typeof (len = elem.length) == "number") for (j = 0; j < len; j++) findInputs(elem[j]); else findInputs(elem);
      ...
      if (fragment) {
        ...
        for (i = 0; ret[i]; i++) {
          script = ret[i];
          if (scripts && jQuery.nodeName(script, "script") && (!script.type || rscriptType.test(script.type))) scripts.push(script.parentNode ? script.parentNode.removeChild(script) : script); else {
  ...
  jQuery.extend({
    ...
    css: function(elem, name, extra) {
      ...
      if (hooks && "get" in hooks && (ret = hooks.get(elem, !0, extra)) !== undefined) return ret;
  ...
  jQuery.each([ "height", "width" ], function(i, name) {
    jQuery.cssHooks[name] = {
      get: function(elem, computed, extra) {
        if (computed) return elem.offsetWidth !== 0 ? getWidthOrHeight(elem, name, extra) : jQuery.swap(elem, cssShow, function() {
    ...
    jQuery.support.reliableMarginRight || (jQuery.cssHooks.marginRight = {
      get: function(elem, computed) {
        return jQuery.swap(elem, {
          ...
          return computed ? curCSS(elem, "margin-right") : elem.style.marginRight;
        });
      }
    });
  });
  ...
  jQuery.ajaxPrefilter("json jsonp", function(s, originalSettings, jqXHR) {
    ...
    if (s.dataTypes[0] === "jsonp" || s.jsonp !== !1 && (jsre.test(s.url) || inspectData && jsre.test(s.data))) {
  ...
  jQuery.fn.extend({
    show: function(speed, easing, callback) {
      ...
      for (var i = 0, j = this.length; i < j; i++) {
        ...
        if (elem.style) {
          ...
          (display === "" && jQuery.css(elem, "display") === "none" || !jQuery.contains(elem.ownerDocument.documentElement, elem)) && jQuery._data(elem, "olddisplay", defaultDisplay(elem.nodeName));
        }
      }
      for (i = 0; i < j; i++) {
        elem = this[i];
    ...
    hide: function(speed, easing, callback) {
      ...
      for (i = 0; i < j; i++) this[i].style && (this[i].style.display = "none");
      return this;
    },
    ...
    animate: function(prop, speed, easing, callback) {
      function doAnimation() {
        ...
        for (p in prop) {
          ...
          if ((hooks = jQuery.cssHooks[name]) && "expand" in hooks) {
            ...
            delete prop[name];
            for (p in replace) p in prop || (prop[p] = replace[p]);
          }
        }
        for (name in prop) {
          val = prop[name];
      ...
      if (jQuery.isEmptyObject(prop)) return this.each(optall.complete, [ !1 ]);
    ...
    stop: function(type, clearQueue, gotoEnd) {
      ...
      return this.each(function() {
        function stopQueue(elem, data, index) {
          ...
          jQuery.removeData(elem, index, !0);
          hooks.stop(gotoEnd);
        }
        var index, hadTimers = !1, timers = jQuery.timers, data = jQuery._data(this);
        gotoEnd || jQuery._unmark(!0, this);
        if (type == null) for (index in data) data[index] && data[index].stop && index.indexOf(".run") === index.length - 4 && stopQueue(this, data, index); else data[index = type + ".run"] && data[index].stop && stopQueue(this, data, index);
  ...
  jQuery.extend({
    speed: function(speed, easing, fn) {
      ...
      opt.complete = function(noUnmark) {
        ...
        opt.queue ? jQuery.dequeue(this, opt.queue) : noUnmark !== !1 && jQuery._unmark(this);
      };
      return opt;
    },
    easing: {
  ...
  jQuery.fx.prototype = {
    ...
    custom: function(from, to, unit) {
      ...
      t.saveState = function() {
        jQuery._data(self.elem, "fxshow" + self.prop) === undefined && (self.options.hide ? jQuery._data(self.elem, "fxshow" + self.prop, self.start) : self.options.show && jQuery._data(self.elem, "fxshow" + self.prop, self.end));
      };
    ...
    step: function(gotoEnd) {
      ...
      if (gotoEnd || t >= options.duration + this.startTime) {
        ...
        for (p in options.animatedProperties) options.animatedProperties[p] !== !0 && (done = !1);
        if (done) {
          options.overflow != null && !jQuery.support.shrinkWrapBlocks && jQuery.each([ "", "X", "Y" ], function(index, value) {
  ...
  jQuery.each(fxAttrs.concat.apply([], fxAttrs), function(i, prop) {
    prop.indexOf("margin") && (jQuery.fx.step[prop] = function(fx) {
  ...
  "getBoundingClientRect" in document.documentElement ? getOffset = function(elem, doc, docElem, box) {
    try {
  ...
  jQuery.fn.offset = function(options) {
    ...
    return doc ? elem === doc.body ? jQuery.offset.bodyOffset(elem) : getOffset(elem, doc, doc.documentElement) : null;
  };
  jQuery.offset = {
    ...
    jQuery.fn[type] = function(value) {
      return jQuery.access(this, function(elem, type, value) {
        ...
        if (elem.nodeType === 9) {
          ...
          return doc[clientProp] >= doc[scrollProp] ? doc[clientProp] : Math.max(elem.body[scrollProp], doc[scrollProp], elem.body[offsetProp], doc[offsetProp]);
        }
        if (value === undefined) {
